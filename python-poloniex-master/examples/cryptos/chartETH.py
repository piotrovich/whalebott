...
class IniciaChart(object):
    def __init__(self):
        Chart('USDT_ETH').ejecuta()


class Chart(IniciaChart):
    ...
    def __init__(self, pair, **kwargs):
        ...
        self.orden_venta_stop, self.stop, self.contador_bollinger_strat, self.contador_cantidad_bbtop = 0, 0, 0, 0
    ...
    def comprobar_init(self):
        try:
            ...
            if float(self.rcb['USDT']['onOrders']):
                ...
                self.roo = returnOpenOrders(self.polo, 'USDT_ETH')
                if self.roo:
            ...
            sleep(1)
            self.tipo_rcb = procesarRCB(self.rcb, 'ETH')
            logger.debug('[comprobar_init] tipo_rcb: ETH, correccion de ventas')
            order = False
            if self.tipo_rcb == 1 or self.tipo_rcb == 2 or self.tipo_rcb == 3:
                ...
                while not order:
                    order = returnTradeHistory(self.polo, 'USDT_ETH', time() - (self.api.DAY * 5))
                    if order and self.tipo_rcb == 1:
                    ...
                    elif order and (self.tipo_rcb == 2 or self.tipo_rcb == 3):
                        logger.debug('[comprobar_init] order and (self.tipo_rcb == 2 or self.tipo_rcb == 3)')
                        self.roo = returnOpenOrders(self.polo, 'USDT_ETH')
                        if self.roo:
                            logger.debug('[comprobar_init] if self.roo:')
                            for rooeth in self.roo:
                                if rooeth['type'] == 'sell':
                                    self.precio_venta = float(rooeth['rate'])
                                    self.orden_venta = int(rooeth['orderNumber'])
                                    break
                            ...
                            self.stop = calcularStop(float(df['bbbottom'].iloc[self.valor_vela_size]),
                                                     self.porcentajeStop)
                            ...
                            if evaluar_stop:
                                ...
                                if senal < 0:
                                    ...
                                    logger.debug('[comprobar_init] La orden no se encuentra en stop ni en senal < 0, '
                                                 'se cancela orden activa y self.enProceso = compra')
    ...
    def whaleBot(self, df):
        ...
        if senal > 0 and self.enProceso == 'hodl':
            ...
            logger.debug('[whaleBot] balance completo ETH: ' + str(self.rcb['ETH']['available']))
            self.rcb = False
        ...
        while True:
            if self.enProceso == 'compra':
                ...
                if evaluar_stop:
                    logger.debug('[whaleBot] evaluar Stop: ' + str(evaluar_stop))
                    self.rcb = returnCompleteBalances(self.polo)
                    self.tipo_rcb = procesarRCB(self.rcb, 'ETH')
                    ...
                    if self.tipo_rcb == 3 or self.tipo_rcb == 2 or self.tipo_rcb == 1:
                        ...
                        if order == 'stop_terminado':
                            ...
                            self.rcb = returnCompleteBalances(self.polo)
                            logger.debug('[whaleBot] balance completo USDT: ' + str(self.rcb['USDT']['available']))
                            logger.debug('[whaleBot] balance completo USDT: ' + str(self.rcb['ETH']['available']))
                            self.rcb = False
                            break
                        elif not order:
                ...
                # logger.debug('[whaleBot] Mantener en HODL')
                break
    ...
    def comprar_whale(self):
        ...
        try:
            ...
            mkt = marketTradeHist(self.polo, 'USDT_ETH', time() - (self.polo.HOUR * 2))
            if mkt:
                ########################################################
            ...
            logger.debug('[comprar_whale] comprar USDT_ETH: [precio_compra: ' + str(nuevo_precio_compra) + '], '
                                                                                                           '[valor_monto: ' + str(
                valor_monto) + ']')
            order = comprar(self.polo, 'USDT_ETH', nuevo_precio_compra, valor_monto)
            ...
            if order:
                ...
                if order['resultingTrades']:
                    ...
                    if amount_for == valor_monto:
                        ...
                        # La compra es parcial, se acelera
    ...
    def vender_whale(self):
        ...
        try:
            ...
            monto = float(self.rcb['ETH']['available'])
            nuevo_precio_venta = calcularFloat(self.valorVela - (self.valorVela * 0.0001))
            logger.debug('[vender_whale] Vender USDT_ETH: [nuevo_precio_venta: ' + str(nuevo_precio_venta) + '], '
                                                                                                             '[valor_monto: ' + str(
                monto) + ']')
            order = vender(self.polo, 'USDT_ETH', nuevo_precio_venta, monto)
            ...
            if order:
                ...
                if order['resultingTrades']:
                    ...
                    if amount_for == monto:
                        ...
                        logger.debug('[vender_whale] Se vende todo en ETH!, [precio_venta: ' + str(
                            self.precio_venta) + ']')
                        self.enProceso = 'hodl'
                        self.stop, self.orden_venta_stop, self.orden_venta, self.precio_venta = 0, 0, 0, 0
                        self.roo, self.tipo_rcb, self.rcb = False, False, False
                        ...
                        # La venta es parcial, se acelera
            ...
            if self.tipo_rcb == 2 or self.tipo_rcb == 3:
                logger.debug('[vender_whale] if self.tipo_rcb == 2 or self.tipo_rcb == 3:, super().__init__()')
                super().__init__()
            if self.enProceso == 'compra':
                self.dataFrame()
                self.vender_whale()
    ...
    def stopLimit(self):
        ...
        try:
            ...
            if (self.tipo_rcb == 1 or self.tipo_rcb == 3) and self.orden_venta_stop == 0:
                ...
                logger.debug('[stopLimit] Vender USDT_ETH: [nuevo_precio_venta_stop: ' + str(nuevo_precio_venta_stop) +
                             '], [monto: ' + str(self.rcb['ETH']['available']) + ']')
                order = vender(self.polo, 'USDT_ETH', nuevo_precio_venta_stop, float(self.rcb['ETH']['available']))
                ...
                if order:
                    ...
                    if order['resultingTrades']:
                        ...
                        for ord in order['resultingTrades']:
                            amount_for += float(ord['amount'])
                        if amount_for == float(self.rcb['ETH']['available']):
                            if self.tipo_rcb == 1:
            ...
            if roo_stop and self.tipo_rcb == 3:
                ...
                if self.precio_venta_stop <= self.valorVela:
                    logger.debug('[stopLimit] self.precio_venta_stop <= self.valorVela:, se repite stopLimit')
                    sleep(0.5)
                    self.no_loop()
                    self.rcb = returnCompleteBalances(self.polo)
                    self.tipo_rcb = procesarRCB(self.rcb, 'ETH')
                ...
                monto_total = float(roo_stop['amount']) + float(self.rcb['ETH']['available'])
                valor_monto = calcularFloat(monto_total)
                logger.debug('[stopLimit] moveOrder USDT_ETH: [orden_venta_stop: ' + str(self.orden_venta_stop) + '], '
                                                                                                                  '[nuevo_precio_venta_stop: ' + str(
                    nuevo_precio_venta_stop) + '], [valor_monto: ' + str(valor_monto) + ']')
                order = moveOrder(self.polo, self.orden_venta_stop, nuevo_precio_venta_stop, valor_monto)
                logger.debug('[stopLimit] Movi!: ' + str(order))
                if order:
                    self.precio_venta_stop = nuevo_precio_venta_stop
                    self.orden_venta_stop = int(order['orderNumber'])
                    if order['resultingTrades']['USDT_ETH']:
                        amount_for = 0
                        for ord in order['resultingTrades']['USDT_ETH']:
                            amount_for += float(ord['amount'])
                        logger.debug('[stopLimit] if order[resultingTrades][USDT_ETH]:, [amount_for: '
                ...
                if order:
                    ...
                    if order['resultingTrades']['USDT_ETH']:
                        ...
                        for ord in order['resultingTrades']['USDT_ETH']:
                            amount_for += float(ord['amount'])
                        logger.debug('[stopLimit] if order[resultingTrades][USDT_ETH]:, [amount_for: '
                                     + str(amount_for) + '], [ETH onOrders: ' + str(self.rcb['ETH']['onOrders']) + ']')
                        if amount_for == float(self.rcb['ETH']['onOrders']):
                            logger.debug('[stopLimit] Se vendieron todos los stopLimit, [precio_venta_stop: ' + str(
                                self.precio_venta_stop) + ']')
                            return 'stop_terminado'
                        else:
                ...
                logger.debug('[stopLimit] elif not roo_stop or self.tipo_rcb == 0:, Se vendieron todos los stopLimit, '
                             '[precio_venta_stop: ' + str(self.precio_venta_stop) + ']')
                return 'stop_terminado'
    ...
    def bollinger_strat(self, df, targetcol='close'):
        ...
        :param contador_cantidad_bbtop: contador que avisa cuando la vela supero al bbtop
        :param contador_cantidad_bbbottom: contador que avisa cuando la vela es menor al bbbottom
        :return: senal
        """
        percent = 0.00025
        percent_contador = 0.00015
        valor_venta = float(df['bbtop'].iloc[self.valor_vela_size]) + (
                float(df['bbtop'].iloc[self.valor_vela_size]) * percent)
        valor_percent_contador = float(df['bbtop'].iloc[self.valor_vela_size]) + (
                float(df['bbtop'].iloc[self.valor_vela_size]) * percent_contador)
        ...
        elif df['low'].iloc[self.valor_vela_size] <= df['bbbottom'].iloc[self.valor_vela_size] and (
    ...
    def comprobarVenta(self):
        ...
        try:
            ...
            if self.roo:
                    ...
                    if (ord['type'] == 'sell') and (self.orden_venta == int(ord['orderNumber'])):
                        ...
                        if self.precio_venta > self.valorVela:
                            ...
                            if move:
                                ...
                                if move['resultingTrades']['USDT_ETH']:
                                    amount_for = 0
                                    for ord2 in move['resultingTrades']['USDT_ETH']:
                                        amount_for += float(ord2['amount'])
                                    logger.debug('[comprobarVenta] if move[resultingTrades][USDT_ETH]:, [amount_for: '
                                                 + str(amount_for) + '], [amount: ' + str(ord['amount']) + ']')
                                    if amount_for == float(ord['amount']):
                                        logger.debug('[comprobarVenta] Se vende todo en ETH!, [precio_venta: ' + str(
                                            self.precio_venta) + ']')
                                        self.enProceso = 'hodl'
                                        self.stop, self.orden_venta_stop, self.orden_venta, self.precio_venta = 0, 0, 0, 0
                                        self.roo, self.tipo_rcb, self.rcb = False, False, False
                                        ...
                                        logger.debug('[comprobarVenta] No se vende todo el ETH, se vuelve a comprobar')
                                        self.no_loop()
                                        self.comprobarVenta()
                                else:
                                    logger.debug('[comprobarVenta] No se vende todo '
                                                 'el ETH, se vuelve a comprobar')
                                    self.no_loop()
                                    self.comprobarVenta()
                            else:
                        ...
                        logger.debug('[comprobarVenta] Se vende todo en ETH!, [precio_venta: '
                ...
                logger.debug('[comprobarVenta] Se vende todo en ETH!, [precio_venta: ' + str(self.precio_venta) + ']')
                self.enProceso = 'hodl'
                self.stop, self.orden_venta_stop, self.orden_venta, self.precio_venta = 0, 0, 0, 0
                self.roo, self.tipo_rcb, self.rcb = False, False, False
                self.precio_compra, self.orden_compra, self.precio_venta_stop = 0, 0, 0
                    ...
                    if (ord['type'] == 'buy') and (self.orden_compra == int(ord['orderNumber'])):
                        ...
                        if self.precio_compra < self.valorVela:
                            ...
                            logger.debug('[comprobarCompra] self.precio_compra < self.valorVela:, '
                                         ...
                                         + str(self.valorVela) + '], [nuevo_precio_compra: ' + str(
                                nuevo_precio_compra) + ']')
                            monto_eth = float(ord['amount'])
                            logger.debug('[comprobarCompra] monto_eth: ' + str(monto_eth))
                            monto_usdt_nuevo = calcularFloat(monto_eth * nuevo_precio_compra)
                            ...
                            if suma_usdt >= monto_usdt_nuevo:
                                ...
                                valor_monto_nuevo = calcular_monto(monto_usdt_nuevo, nuevo_precio_compra)
                            ...
                            if move:
                                ...
                                if move['resultingTrades']['USDT_ETH']:
                                    amount_for = 0
                                    for ord in move['resultingTrades']['USDT_ETH']:
                                        amount_for += float(ord['amount'])
                                    logger.debug('[comprobarCompra] if move[resultingTrades][USDT_ETH]:, [amount_for: '
                                                 + str(amount_for) + '], [monto_eth: ' + str(monto_eth) + ']')
                                    if amount_for == monto_eth:
                                        ...
                                        logger.debug('[comprobarCompra] '
                                                     'No se compra todo el ETH, se vuelve a comprobar')
                                        self.no_loop()
                                        self.comprobarCompra()
                                else:
                                    logger.debug('[comprobarCompra] No se compra todo el ETH, se vuelve a comprobar')
                                    self.no_loop()
                                    self.comprobarCompra()
                            else:
...
if __name__ == '__main__':
    ...
    handler = logging.handlers.TimedRotatingFileHandler(filename='examples/cryptos/logs/ChartETH.log', when="h",
                                                        interval=8, backupCount=50)
    # HANDLER PARA PYCHARM
    # handler = logging.handlers.TimedRotatingFileHandler(filename='logs/Chart.log', when="h", interval=8,
    #                                                    backupCount=50)
